...
<li class="nav-item m-1 @if($menu->parent_id === 0 && count($menu->children) > 0) dropdown @endif">
    <a href="/menu/{{ $menu->slug }}" class="nav-link dropdown-toggle" @if($menu->parent_id === 0 && count($menu->children) > 0) role="button" id="dropdown" @endif>
        {{ $menu->title }} 
...
@if (count($menu->children) > 0)
    <ul class="dropdown-menu" aria-labelledby="dropdownBtn">
        @foreach($menu->children as $menu)
            @include('testDynamicNavbar.submenu', $menu)
        @endforeach
    </ul>
@endif
</li>
